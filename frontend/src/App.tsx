...
function SettingsPage({ value, config, onSaved, onError }: { value: Settings; config: any; onSaved: () => void; onError: (v: string) => void }) {
  ...
  const download = async () => { const payload = await api<any>('/export'); const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = exportFilename(); anchor.click(); URL.revokeObjectURL(url) }
  ...
  const chooseFile = async (file?: File) => { if (!file) return; try { const text = await file.text(); const payload = JSON.parse(text); const count = targetCount(payload); if (count == null) throw new Error('This file does not contain a SatScheduler targets array.'); setImportText(JSON.stringify(payload, null, 2)); setImportFileName(`${file.name} · ${count} targets`) } catch (error) { setImportText(''); setImportFileName(''); onError(`Cannot read import file: ${String(error)}`) } }
  const importConfig = async () => { const payload = JSON.parse(importText), count = targetCount(payload); if (count == null) throw new Error('JSON does not contain a SatScheduler targets array.'); if (!confirm(`Import ${count} targets for station ${config?.station?.station_id || '—'} and replace the current watch list? Targets for other stations will be skipped.`)) return; const result = await api<{ imported: number; skipped_station_mismatch: number }>('/import?replace=true', { method: 'POST', body: JSON.stringify(payload) }); setImportText(''); setImportFileName(`Imported ${result.imported} · skipped ${result.skipped_station_mismatch} for station mismatch`); onSaved() }
