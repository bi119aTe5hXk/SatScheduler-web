...
function TargetEditor({ value, onClose, onSaved, onError }: { value?: Target; onClose: () => void; onSaved: () => void; onError: (v: string) => void }) {
  const [satellites, setSatellites] = useState<any[]>([]), [transmitters, setTransmitters] = useState<TransmitterInsight[]>([]), [loadingTransmitters, setLoadingTransmitters] = useState(false)
  ...
  useEffect(() => { api<any>('/satellites').then(v => setSatellites(v.results || [])).catch(e => onError(String(e))) }, [])
  ...
  return <div className="modal-backdrop"><div className="modal"><div className="panel-title"><div><small>WATCH TARGET</small><h2>{value ? 'Edit satellite' : 'Add satellite'}</h2></div><button className="ghost" onClick={onClose}>Close</button></div>
    <div className="form-grid"><label>Satellite<select value={form.sat_id} onChange={e => setForm({ ...form, sat_id: e.target.value, transmitter_uuid: '', transmitter_success_rate: null, transmitter_good_count: null, transmitter_max_good_count: null })}><option value="">Select…</option>{satellites.map(s => <option value={s.sat_id} key={s.sat_id}>{s.name} · {s.norad_cat_id}</option>)}</select></label>
      <label>Minimum horizon °<input type="number" value={form.min_elevation ?? ''} onChange={e => number('min_elevation', e.target.value)} /></label><label>Minimum peak °<input type="number" value={form.min_peak_elevation ?? ''} onChange={e => number('min_peak_elevation', e.target.value)} /></label><label>Maximum peak °<input type="number" value={form.max_peak_elevation ?? ''} onChange={e => number('max_peak_elevation', e.target.value)} /></label><label>Priority 0–1<input type="number" step="0.1" min="0" max="1" value={form.priority} onChange={e => number('priority', e.target.value)} /></label><label>Azimuth from °<input type="number" value={form.min_azimuth ?? ''} onChange={e => number('min_azimuth', e.target.value)} /></label><label>Azimuth to °<input type="number" value={form.max_azimuth ?? ''} onChange={e => number('max_azimuth', e.target.value)} /></label></div>
