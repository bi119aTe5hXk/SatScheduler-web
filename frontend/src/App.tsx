...
function Schedule({ settings, targets, onNotify }: { settings: Settings; targets: Target[]; onNotify: Notify }) {
  ...
  const activePlanJob = useRef<string | null>(null), activeScheduleJob = useRef<string | null>(null), loadedPlanJob = useRef<string | null>(null)
  ...
  return <div className="page"><PageHeader eyebrow="PLANNING DESK" title="Build an observation plan" action={<button className="primary" disabled={loading || submitting || !targets.length} onClick={build}>{loading ? 'Calculating…' : 'Recalculate passes'}</button>} />
    <section className="hero-grid"><Metric label="Engine" value={settings.prediction_engine === 'satnogs_predict' ? 'SatNOGS' : 'Skyfield'} detail={settings.comparison_enabled ? 'Comparison enabled' : 'Primary only'} /><Metric label="Sort mode" value={settings.sort_mode.replaceAll('_', ' ')} detail="Configured in Settings" /><Metric label="Planning horizon" value={`${settings.horizon_hours}H`} detail="All non-conflicting passes" /><Metric label="Enabled targets" value={String(targets.filter(t => t.enabled).length)} detail={`Limit ${settings.satellites_per_run} satellites / run`} /></section>
...
function SettingsPage({ value, config, onSaved, onNotify }: { value: Settings; config: any; onSaved: () => void; onNotify: Notify }) {
  ...
  return <div className="page"><PageHeader eyebrow="SYSTEM CONFIGURATION" title="Scheduler settings" action={<button className="primary" onClick={() => save().catch(e => onNotify(String(e), 'error'))}>Save settings</button>} />
    ...
    <section className="split"><div className="panel"><div className="panel-title"><h2>Batch and API policy</h2></div><div className="form-grid"><NumberField label="Satellites / run" help="Maximum number of different satellites considered in one plan. Every non-conflicting pass for those satellites is selected. The current limit is 30." value={form.satellites_per_run} min={1} max={30} onChange={v => set('satellites_per_run', v)} /><NumberField label="API batch size" help="Number of observations sent in each SatNOGS scheduling request. Smaller batches isolate failures but require more requests." value={form.batch_size} min={1} max={50} onChange={v => set('batch_size', v)} /><NumberField label="API request interval seconds" help="Minimum delay shared by all SatNOGS DB and Network requests, including pagination. 3–5 seconds is recommended to avoid HTTP 429." value={form.api_request_interval_seconds} min={0.5} max={30} step={0.5} onChange={v => set('api_request_interval_seconds', v)} /><NumberField label="Conflict buffer seconds" help="Adds this safety margin before and after existing reservations when checking whether a predicted pass conflicts." value={form.conflict_buffer_seconds} min={0} max={3600} onChange={v => set('conflict_buffer_seconds', v)} /><NumberField label="Problem threshold" help="After this many consecutive scheduling failures, the affected watch target is marked as a problem for review." value={form.problem_threshold} min={1} max={20} onChange={v => set('problem_threshold', v)} /></div><label className="check setting-check"><input type="checkbox" checked={form.retry_individually} onChange={e => set('retry_individually', e.target.checked)} /><span>Retry failed batches one observation at a time<small className="field-help">If a batch request fails, resubmit each observation separately so one bad satellite does not block the rest.</small></span></label></div>
