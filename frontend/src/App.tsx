...
function Dashboard({ config, targets, onNavigate, onNotify }: { config: any; targets: Target[]; onNavigate: (p: Page, observationId?: number) => void; onNotify: Notify }) {
  ...
  useEffect(() => {
    setUpcoming(current => {
      const nextItems = activeUpcoming(current, now.getTime()).sort((a, b) => new Date(a.start || 0).getTime() - new Date(b.start || 0).getTime())
      return nextItems.length === current.length && nextItems.every((item, index) => item.id === current[index]?.id) ? current : nextItems
    })
  }, [now])
...
function observationMatches(item: Observation, query: string, targets: Target[]): boolean {
...
function MiniPolarPlot({ observation }: { observation: Observation }) {
  const center = 32, radius = 26
  const point = (azimuth: number, elevation: number) => { const angle = (azimuth - 90) * Math.PI / 180, distance = radius * (1 - elevation / 90); return { x: center + distance * Math.cos(angle), y: center + distance * Math.sin(angle) } }
  const riseAzimuth = observation.rise_azimuth ?? 0, setAzimuth = observation.set_azimuth ?? 180
  const delta = ((setAzimuth - riseAzimuth + 540) % 360) - 180, peakAzimuth = (riseAzimuth + delta / 2 + 360) % 360
  const rise = point(riseAzimuth, 0), peak = point(peakAzimuth, observation.max_altitude ?? 0), set = point(setAzimuth, 0)
  const control = { x: 2 * peak.x - (rise.x + set.x) / 2, y: 2 * peak.y - (rise.y + set.y) / 2 }
  return <svg className="mini-polar" viewBox="0 0 64 64" aria-hidden="true"><circle className="mini-polar-horizon" cx={center} cy={center} r={radius} /><path d={`M ${rise.x} ${rise.y} Q ${control.x} ${control.y} ${set.x} ${set.y}`} /><circle className="rise" cx={rise.x} cy={rise.y} r="2.4" /><circle className="peak" cx={peak.x} cy={peak.y} r="2.7" /><circle className="set" cx={set.x} cy={set.y} r="2.4" /></svg>
}
...
function Timeline({ observations, now }: { observations: Observation[]; now?: Date }) {
  const start = now?.getTime() ?? Date.now(), span = 48 * 3600_000
...
function Targets({ targets, onChanged, onNotify }: { targets: Target[]; onChanged: () => void; onNotify: Notify }) {
  ...
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set()), [draggingId, setDraggingId] = useState<string | null>(null)
  const reorder = async (next: Target[], message = 'Satellite priority order updated.') => { await api('/targets/reorder', { method: 'PUT', body: JSON.stringify({ ids: next.map(t => t.id) }) }); await onChanged(); onNotify(message, 'success') }
  ...
  return <div className="page"><PageHeader eyebrow="WATCH TARGETS" title="Satellite list" action={<div className="button-row"><button className="danger" disabled={!selectedIds.size} onClick={() => deleteSelected().catch(error => onNotify(String(error), 'error'))}>Delete selected ({selectedIds.size})</button><button className="ghost" disabled={!selectedIds.size} onClick={() => moveSelectedToTop().catch(error => onNotify(String(error), 'error'))}>Move selected to top ({selectedIds.size})</button><button className="primary" onClick={() => setEditing('new')}>Add satellite</button></div>} />
    ...
    <div className="panel target-table">{targets.length === 0 && <div className="empty">No satellites yet. Add the first watch target.</div>}{targets.map((target, index) => <div className={`target-row ${draggingId === target.id ? 'dragging' : ''}`} key={target.id} onDragOver={event => { if (draggingId) { event.preventDefault(); event.dataTransfer.dropEffect = 'move' } }} onDrop={event => dropTarget(event, target.id).catch(error => onNotify(String(error), 'error'))}>
      ...
      <div className="order-controls"><span className="drag-handle" draggable onDragStart={event => { setDraggingId(target.id); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', target.id) }} onDragEnd={() => setDraggingId(null)} title="Drag to reorder">⋮⋮</span><button onClick={() => move(index, -1).catch(error => onNotify(String(error), 'error'))}>↑</button><span>{String(index + 1).padStart(2, '0')}</span><button onClick={() => move(index, 1).catch(error => onNotify(String(error), 'error'))}>↓</button></div>
      <div className="target-main"><div><strong>{target.satellite_name || target.name}</strong><small>{target.sat_id} · NORAD {target.norad_cat_id || '—'}</small></div><div className="tags">{target.requires_station_daylight && <span>Station daylight</span>}{target.min_peak_elevation != null && <span>Peak ≥ {target.min_peak_elevation}°</span>}{target.max_peak_elevation != null && <span>Peak ≤ {target.max_peak_elevation}°</span>}{target.min_azimuth != null && <span>Az {target.min_azimuth}–{target.max_azimuth}°</span>}</div></div>
